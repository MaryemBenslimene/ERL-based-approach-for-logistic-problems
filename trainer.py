...
def train_tsp(args):
    ...
    GAOptim = GA.GAOptimizer(generations=50, mutation_rate=0.1, population_size=100)
    ...
    for generation in range(GAOptim.generations):
        ...
        for individual in GAOptim.population :
            ...
            out = validate(test_loader, individual, tsp.reward, tsp.render, test_dir, num_plot=5)

            GAOptim.fitness(out, fitness)
        parents = GAOptim.parents_selection(fitness)
        offsprings = GAOptim.crossover(parents)
        GAOptim.population = parents + offsprings
        GAOptim.population = GAOptim.mutation()
